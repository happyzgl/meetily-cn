...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert(t('recording.failedToInit'));
      }
    };
    checkTauri();
  }, []);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        setDeviceError({
          title: t('recording.micNotAvailable'),
          message: t('recording.micNotAvailableMsg')
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')) {
        setDeviceError({
          title: t('recording.systemAudioNotAvailable'),
          message: t('recording.systemAudioNotAvailableMsg')
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: t('recording.permissionRequired'),
          message: t('recording.permissionRequiredMsg')
        });
      } else {
        setDeviceError({
          title: t('recording.recordingFailed'),
          message: t('recording.recordingFailedMsg')
        });
      }
    }
  ...
  const handlePauseRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('recording.failedToPause'));
    } finally {
      setIsPausing(false);
    }
  ...
  const handleResumeRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('recording.failedToResume'));
    } finally {
      setIsResuming(false);
    }
                          ...
                          >
                            ...
                            {(isPausing || isResuming) && (
                              <div className="absolute -top-8 text-gray-600 font-medium text-xs">
                                {isPausing ? t('misc.updating') : t('misc.updating')}
                              </div>
                            )}
                          </button>
                        </TooltipTrigger>
