...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('transcriptSettings.audioDeviceSettings'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError(t('transcriptSettings.noMicDevices'));
        return;
      }
      ...
      setError(t('recordingSettings.failedToStart'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('transcriptSettings.audioDevices')}</h4>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder={t('transcriptSettings.defaultMic')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('transcriptSettings.defaultMic')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('transcriptSettings.noMicDevices')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('transcriptSettings.micLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={t('transcriptSettings.defaultSystemAudio')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('transcriptSettings.defaultSystemAudio')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('transcriptSettings.noSystemAudioDevices')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('transcriptSettings.defaultMic')}:</strong> {t('transcriptSettings.micConnected')}</p>
        <p>• <strong>{t('transcriptSettings.defaultSystemAudio')}:</strong> ...</p>
        {isMonitoring && (
          <p>• <strong>{t('transcriptSettings.micLevels')}</strong> {t('transcriptSettings.greenYellowRed')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• {t('common.tip')} {t('transcriptSettings.clickTestMic')}</p>
        )}
      </div>
    </div>
  );
