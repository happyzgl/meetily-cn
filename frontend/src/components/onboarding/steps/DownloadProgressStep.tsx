...
export function DownloadProgressStep() {
  ...
  const handleRetryDownload = async () => {
    ...
    try {
      ...
      setParakeetState((prev) => ({
        ...prev,
        status: 'error',
        error: error instanceof Error ? error.message : t('common.failed'),
      }));

      toast.error(t('onboarding.downloadError'), {
        description: t('onboarding.downloadError'),
      });
    } finally {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (actuallyAvailable && !parakeetDownloaded) {
        ...
        toast.error(t('transcriptSettings.engineRequired'), {
          description: t('onboarding.retryDownload'),
        });
        return;
      }
    } catch (error) {
    ...
    if (!downloadsComplete) {
      toast.info(t('onboarding.downloadWillContinueInBackground'), {
        description: t('onboarding.startUsing'),
        duration: 5000,
      });
    }
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div>
          {state.status === 'waiting' && (
            <span className="text-sm text-gray-500">{t('misc.waiting')}</span>
          )}
          ...
          {state.status === 'error' && (
            <span className="text-sm text-red-500">{t('common.failed')}</span>
          )}
      ...
      {(state.status === 'error' || state.status === 'cancelled') && (
        <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
          <p className="text-sm text-red-600 font-medium">
            {state.status === 'cancelled' ? t('onboarding.downloadCancelled') : t('onboarding.downloadError')}
          </p>
          {state.error && <p className="text-xs text-red-500 mt-1">{state.error}</p>}
          {(title === 'Transcription Engine' || title === 'Summary Engine') && (
            <button
              onClick={title === t('onboarding.downloadProgressTitle') ? handleRetryDownload : handleRetrySummaryDownload}
  ...
  return (
    <OnboardingContainer
      ...
      description={t('onboarding.needModelFirst')}
      step={3}
    ...
    >
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full max-w-lg space-y-4">
          {renderDownloadCard(
            t('onboarding.downloadProgressTitle'),
          ...
          {parakeetDownloaded && !summaryModelDownloaded && (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium">{t('onboarding.continueWhileDownloading')}</p>
                  <p className="text-gray-700 mt-1">
                    {t('onboarding.downloadWillContinueInBackground')}
                  </p>
                </div>
              </div>
            </motion.div>
