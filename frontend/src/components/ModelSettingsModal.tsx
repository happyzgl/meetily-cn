...
export function ModelSettingsModal({
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    if (trimmedEndpoint && !validateOllamaEndpoint(trimmedEndpoint)) {
      const errorMsg = t('modelSettings.connectionFailed');
      setError(errorMsg);
      if (!silent) {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.failedToLoadModels');
      setError(errorMsg);
  ...
  const loadOpenRouterModels = async () => {
    ...
    try {
      ...
      setOpenRouterError(
        err instanceof Error ? err.message : t('modelSettings.failedToLoadModels')
      );
    } finally {
  ...
  const handleSave = async () => {
    ...
    if (modelConfig.provider === 'custom-openai') {
      try {
        ...
        toast.error(t('modelSettings.connectionFailed'));
        return;
      }
    }
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error(t('modelSettings.endpointUrl'));
      return;
    }
    ...
    try {
      ...
      toast.success(result.message || t('modelSettings.connectionSuccess'));
    } catch (err) {
  ...
  const downloadRecommendedModel = async () => {
    ...
    if (isDownloading(recommendedModel)) {
      toast.info(`${recommendedModel} ${t('misc.waiting')}`, {
        description: `${t('common.loading')} ${Math.round(getProgress(recommendedModel) || 0)}%`
      });
      return;
    }
    ...
    try {
      ...
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error(t('modelSettings.ollamaNotRunning'), {
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      ...
      toast.success(`${t('transcriptSettings.transcriptionEngine')} ${modelName} ${t('common.deleted')}`);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold">{t('modelSettings.title')}</h3>
      </div>

      <div className="space-y-4">
        <div>
          <Label>{t('summarySettings.title')}</Label>
          <div className="flex space-x-2 mt-1">
            ...
            >
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">{t('modelSettings.builtInAI')}</SelectItem>
                <SelectItem value="claude">Claude</SelectItem>
                <SelectItem value="custom-openai">{t('modelSettings.customServer')}</SelectItem>
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || t('modelSettings.modelName')}
                    </span>
                      ...
                      {(modelConfig.provider === 'openrouter' && isLoadingOpenRouter) ||
                       ...
                       (modelConfig.provider === 'groq' && isLoadingGroq) ? (
                        <div className="py-6 text-center text-sm text-muted-foreground">
                          ...
                          {t('modelSettings.fetchingModels')}
                        </div>
                      ) : (
                        <>
                          <CommandEmpty>{t('modelSettings.noModelsFound')}</CommandEmpty>
                          <CommandGroup>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            <div>
              <Label htmlFor="custom-endpoint">{t('modelSettings.endpointUrl')}</Label>
              <Input
                id="custom-endpoint"
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t('modelSettings.baseUrlDesc')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-model">{t('modelSettings.modelName')}</Label>
              <Input
                id="custom-model"
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                {t('modelSettings.modelNameDesc')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-api-key">{t('modelSettings.apiKeyOptional')}</Label>
              <Input
                id="custom-api-key"
                type="password"
                value={customOpenAIApiKey}
                onChange={(e) => setCustomOpenAIApiKey(e.target.value)}
                placeholder={t('modelSettings.leaveEmptyEndpoint')}
                className="mt-1"
              />
            </div>
            ...
            <div>
              ...
              >
                <Label className="cursor-pointer">{t('modelSettings.advancedOptions')}</Label>
              ...
              {isCustomOpenAIAdvancedOpen && (
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    <Label htmlFor="custom-max-tokens">{t('modelSettings.maxTokens')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-temperature">{t('modelSettings.temperature')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-top-p">{t('modelSettings.topP')}</Label>
                    <Input
                      id="custom-top-p"
                      type="number"
                      step="0.1"
            ...
            >
              {isTestingConnection ? (
                <>
                  ...
                  {t('modelSettings.testingConnection')}
                </>
              ) : (
                <>
        ...
        {requiresApiKey && (
          <div>
            <Label>{t('modelSettings.apiKey')}</Label>
            <div className="relative mt-1">
              <Input
                ...
                placeholder={t('modelSettings.apiKeyOptional')}
                className="pr-24"
              />
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            ...
            >
              <Label className="cursor-pointer">{t('modelSettings.customEndpoint')}</Label>
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  {t('modelSettings.leaveEmptyEndpoint')}
                </p>
            ...
            {isLoadingOllama ? (
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          {isDownloading('gemma3:1b') ? (
                            <>
                              <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                              {t('onboarding.downloadingUpdate')} gemma3:1b...
                            </>
                          ) : (
                            <>
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{t('onboarding.downloadingUpdate')} gemma3:1b</span>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {t('modelSettings.noModelsSearchMatch', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                        ...
                        >
                          ...
                          {modelIsDownloading && progress !== undefined && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-blue-600">{t('onboarding.downloadingUpdate')}</span>
